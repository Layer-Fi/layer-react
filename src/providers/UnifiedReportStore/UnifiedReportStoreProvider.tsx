...
import { AccountingMethod, DateGroupBy, type DateQueryParams, type DateRangeQueryParams, type UnifiedReportColumn } from '@schemas/reports/unifiedReport'
...
type UnifiedReportStoreActions = {
  ...
  setAccountingMethod: (accountingMethod: AccountingMethod) => void
...
export type ReportControlParams = {
  [ReportControl.AccountingMethod]: { accountingMethod: AccountingMethod }
...
export type UnifiedReportControlParams = Partial<
  & ReportControlParams[ReportControl.AccountingMethod]
...
export function useUnifiedReportAccountingMethodParam() {
  const store = useContext(UnifiedReportStoreContext)

  const accountingMethod = useStore(store, state => state.accountingMethod)
  const setAccountingMethod = useStore(store, state => state.actions.setAccountingMethod)

  return useMemo(() => ({ accountingMethod, setAccountingMethod }), [accountingMethod, setAccountingMethod])
}
...
export function useUnifiedReportParams(): UnifiedReportParams | null {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      ...(hasControl(report, ReportControl.AccountingMethod) && { accountingMethod }),
  ...
  }, [accountingMethod, effectiveDate, endDate, groupBy, report, startDate, yearStartDate])
}
...
const createUnifiedReportStore = (dateSelectionMode: DateSelectionMode) =>
  createStore<UnifiedReportStoreShape>(set => ({
    ...
    actions: {
      ...
      setAccountingMethod: (accountingMethod: AccountingMethod) => set({ accountingMethod }),
