...
export type DateState = DateRange
...
type DateStore = DateState & { actions: DateActions }

function buildDefaultInitialRange(now: Date): DateRange {
  return {
...
type MakeDateStoreOptions = {
  buildInitialRange?: (now: Date) => DateRange
}
...
export function makeDateStore({ buildInitialRange = buildDefaultInitialRange }: MakeDateStoreOptions = {}) {
  function buildStore() {
    const now = new Date()
  ...
  const DateStoreContext = createContext(buildStore())
  ...
  function DateStoreProvider({ children }: PropsWithChildren) {
