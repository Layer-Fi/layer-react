...
export const validateCustomerForm = ({ customer }: { customer: CustomerForm }, t: TFunction) => {
  ...
  if (!individualName.trim() && !companyName.trim()) {
    errors.push({ individualName: t('customerVendor.eitherIndividualNameOrCompanyNameIsRequired', 'Either individual name or company name is required.') })
  }

  if (!email.trim()) {
    errors.push({ email: t('customerVendor.emailIsARequiredField', 'Email is a required field.') })
  }
