...
export const ProfitAndLossChartWideSelection = ({
  ...
  const selectedStyle = (entry: any) => {
    const selected = theData?.find(e => e.selected)
    const selectedQuarter =
      selected &&
      getQuarter(new Date(Number(selected.year), Number(selected.month) - 1, 1))
    if (period === 'month' && entry.selected) {
      return {
        opacity: 1,
      }
    }

    if (period === 'quarter' && selected) {
      const entryQuarter = getQuarter(
        new Date(Number(entry.year), Number(entry.month - 1), 1),
      )
      if (entryQuarter === selectedQuarter && entry.year === selected.year) {
        return {
          opacity: 1,
        }
      }
    ...
    if (period === 'year' && selected) {
      if (selected.year === entry.year) {
        return {
          opacity: 1,
        }
      }
  ...
  const CustomizedCursor = (props: any) => {
    ...
    const mult = period === 'year' ? 12 : period === 'quarter' ? 3.5 : 1
    return (
      <Rectangle
        fill='#eee'
        stroke='none'
        x={points[0].x - width}
        y={points[0].y}
        width={(props.left - props.right) * mult}
        height={height}
        radius={6}
            ...
            <LabelList
              content={
                <Indicator
                  ...
                  mult={period === 'year' ? 24 : period === 'quarter' ? 7 : 1}
                />
              }
            />
            {theData?.map(entry => {
          ...
          >
            {theData?.map(entry => {
              return (
                <Cell
                  key={entry.name}
                  fill='url(#layer-bar-stripe-pattern)'
                  style={selectedStyle(entry)}
                />
              )
            })}
