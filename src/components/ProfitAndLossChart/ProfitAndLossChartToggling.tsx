...
export const ProfitAndLossChartToggling = ({
  ...
  const theData = useMemo(() => {
    ...
    return data
      ...
      ?.map(x => {
        return {
          ...x,
          quarter: getQuarter(new Date(Number(x.year), Number(x.month) - 1, 1)),
        }
      })
      ?.reduce((acc, x) => {
        if (period === 'month') {
        ...
        if (period === 'quarter') {
          const found = acc.find(
            y => y.year === x.year && y.quarter === x.quarter,
          )
          if (found) {
            found.income = x.income + found.income
            found.totalExpenses = x.totalExpenses + found.totalExpenses
            return acc
          } else {
            acc.push(x)
            return acc
        ...
        if (period === 'year') {
          const found = acc.find(y => y.year === x.year)
          if (found) {
            found.income = x.income + found.income
            found.totalExpenses = x.totalExpenses + found.totalExpenses
            return acc
  ...
  }, [selectionMonth, chartWindow, data, loaded, compactView, period])
