...
export const AddToCalendar = ({
  ...
  const Trigger = useCallback(() => {
    return (
      <Button variant='outlined'>
        <Calendar size={16} />
        {t('callBookings:action.add_calendar', 'Add to Calendar')}
        <ChevronDown size={16} />
      </Button>
    )
  }, [t])
  ...
  return (
    <>
      ...
      <DropdownMenu
        ariaLabel={t('callBookings:action.add_calendar', 'Add to Calendar')}
        slots={{ Trigger }}
        slotProps={{
          Dialog: { width: '10rem' },
        }}
      >
        <MenuList>
          <MenuItem key='google' onClick={() => handleCalendarClick('google')}>
            <HStack gap='sm'>
              <img className='Layer__AddToCalendar__CalendarIcon' src={GoogleIcon} alt={t('callBookings:label.google_calendar', 'Google Calendar')} />
              <Span size='sm'>{t('callBookings:label.google_calendar', 'Google Calendar')}</Span>
            </HStack>
          </MenuItem>
          <MenuItem key='outlook' onClick={() => handleCalendarClick('outlook')}>
            <HStack gap='sm'>
              <img className='Layer__AddToCalendar__CalendarIcon' src={OutlookIcon} alt={t('callBookings:label.outlook_calendar', 'Outlook Calendar')} />
              <Span size='sm'>Outlook</Span>
            </HStack>
          </MenuItem>
          <MenuItem key='yahoo' onClick={() => handleCalendarClick('yahoo')}>
            <HStack gap='sm'>
              <img className='Layer__AddToCalendar__CalendarIcon' src={YahooIcon} alt={t('callBookings:label.yahoo_calendar', 'Yahoo Calendar')} />
          ...
          <MenuItem key='ics' onClick={() => handleCalendarClick('ics')}>
            <HStack gap='sm'>
              <Download size={16} />
              <Span size='sm'>{t('callBookings:action.download_ics_file', 'Download .ics file')}</Span>
            </HStack>
          </MenuItem>
        </MenuList>
      </DropdownMenu>
