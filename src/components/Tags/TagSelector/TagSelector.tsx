...
import { useTagDimensions } from '@hooks/api/businesses/[business-id]/tags/dimensions/useTagDimensions'
import { useFlattenedTagValues } from '@hooks/features/tags/useFlattenedTagValues'
...
function TagSelectorSelection({
  ...
  return (
    <TagGroup
      aria-label={t('tags:activeTags', 'Active Tags')}
      selectionMode='none'
      onRemove={
        onRemoveTag
          ? (keys) => {
  ...
  const EmptyMessage = useMemo(
    () => (
      <Span
        ...
        nonAria
      >
        {t('tags:noMatchingTagsFound', 'No matching tags found')}
      </Span>
    ),
    [t],
  )
  const ErrorMessage = useMemo(
    () => (
      <Span
        ...
        nonAria
      >
        {t('tags:anErrorOccurredWhileLoadingTagOptions', 'An error occurred while loading tag options.')}
      </Span>
    ),
    [t],
  )
    ...
    <LabeledTagSelectorContainer forInputId={inputId}>
      <TagSelectorLayoutGroup>
        <ComboBox
          ...
          placeholder={t('tags:addATagToThisTransaction', 'Add a tag to this transaction...')}
          slots={{
            EmptyMessage,
            ErrorMessage,
          }}
