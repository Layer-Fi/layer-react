...
export const validateInvoicePaymentForm = ({ invoicePayment, invoice }: { invoicePayment: DedicatedInvoicePaymentForm, invoice: Invoice }) => {
  ...
  if (!BD.isPositive(amount)) {
    errors.push({ amount: i18next.t('paymentAmountMustBeGreaterThanZero', 'Payment amount must be greater than zero.') })
  }

  if (BD.greaterThan(amount, convertCentsToBigDecimal(invoice.outstandingBalance))) {
    errors.push({ amount: i18next.t('paymentAmountCannotBeGreaterThanTheOutstandingInvoiceBalance', 'Payment amount cannot be greater than the outstanding invoice balance.') })
  }

  if (paidAt === null) {
    errors.push({ paidAt: i18next.t('paymentDateIsARequiredField', 'Payment date is a required field.') })
  }
  ...
  if (paidAt && toCalendarDate(paidAt).compare(today(getLocalTimeZone())) > 0) {
    errors.push({ paidAt: i18next.t('paymentDateCannotBeInTheFuture', 'Payment date cannot be in the future.') })
  }
