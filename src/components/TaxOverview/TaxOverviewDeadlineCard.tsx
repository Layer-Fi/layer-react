...
import { type TaxOverviewDeadline, type TaxOverviewDeadlineReview, type TaxOverviewDeadlineStatus } from '@schemas/taxEstimates/overview'
...
import './taxOverviewDeadlineCard.scss'
...
export type TaxOverviewDeadlineCardProps = {
  data: TaxOverviewDeadline
      ...
      <HStack className='Layer__TaxOverview__DeadlineRow' justify='space-between' align='start' gap='md'>
        <VStack className='Layer__TaxOverview__DeadlineContent' gap='3xs'>
          ...
          <Span size='sm' variant='subtle'>
            {t('taxEstimates:label.due_with_date', 'Due: {{date}}', { date: formatDate(data.dueAt, DateFormat.DateShort) })}
          </Span>
        </VStack>
        <VStack className='Layer__TaxOverview__DeadlineAmountColumn' align='end' gap='xs'>
          <VStack align='end' gap='3xs'>
            <HStack className='Layer__TaxOverview__DeadlineValueRow' align='center' gap='xs'>
              <TaxOverviewDeadlineStatusIcon status={data.status} />
            ...
            <VStack className='Layer__TaxOverview__DeadlineAmountCopy' align='end' gap='3xs'>
              <Span size='xs' variant='subtle'>{data.description}</Span>
            </VStack>
          </VStack>
        </VStack>
      </HStack>
      {reviewAction && (
        <Stack direction={isDesktop ? 'row' : 'column'} className='Layer__TaxOverview__DeadlineReviewRow' justify='space-between' align='center' gap='md'>
