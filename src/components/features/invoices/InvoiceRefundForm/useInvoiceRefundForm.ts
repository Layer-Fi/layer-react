...
} from '@features/invoices/InvoiceRefundForm/formUtils'
  ...
  const getErrorText = useCallback((reason: InvoiceRefundInvalidReason): string => {
    switch (reason) {
      case InvoiceRefundInvalidReason.CompletedAtRequired:
        return t('invoices:InvoiceRefundForm.useInvoiceRefundForm.validation.refund_date_required', 'Refund date is a required field.')
      case InvoiceRefundInvalidReason.CompletedAtBeforeLastPayment:
        return t('invoices:InvoiceRefundForm.useInvoiceRefundForm.validation.refund_date_before_last_payment', 'Refund date cannot be before the last invoice payment ({{lastPaymentDate}}).', {
      ...
      case InvoiceRefundInvalidReason.CompletedAtInFuture:
        return t('invoices:InvoiceRefundForm.useInvoiceRefundForm.validation.refund_date_future', 'Refund date cannot be in the future.')
      case InvoiceRefundInvalidReason.MethodRequired:
        return t('invoices:InvoiceRefundForm.useInvoiceRefundForm.validation.payment_method_required', 'Payment method is a required field.')
      default:
        return ''
    }
