...
import { useInvoiceDetail, useInvoiceNavigation } from '@providers/features/invoices/InvoicesRouteStore/InvoicesRouteStoreProvider'
...
export const InvoiceDetailHeaderMenu = ({ onEditInvoice }: InvoiceDetailHeaderMenuProps) => {
  ...
  const { trigger: downloadInvoicePdf, isMutating: isDownloadingInvoicePdf } = useGetInvoicePdfDownload({
    ...
    onSuccess: ({ presignedUrl, fileName }) => {
      ...
      addToast({ content: t('invoices:InvoiceDetailHeader.label.download_successful', 'Download successful'), type: 'success' })
    },
    onError: () => addToast({ content: t('invoices:InvoiceDetailHeader.error.download_failed', 'Download failed'), type: 'error' }),
  })

  const onDownloadInvoicePdf = () => {
    addToast({ content: t('invoices:InvoiceDetailHeader.label.download_started', 'Download started') })
  ...
  return (
    <>
      <DropdownMenu
        ariaLabel={t('invoices:InvoiceDetailHeader.label.additional_invoice_actions', 'Additional invoice actions')}
        slots={{ Trigger }}
        ...
        <MenuList>
          ...
          {invoiceActions.includes(InvoiceDetailHeaderMenuActions.Refund) && (
            <MenuItem key={InvoiceDetailHeaderMenuActions.Refund} onClick={onOpenChangeByMode(InvoiceDetailHeaderMenuActions.Refund)}>
              <Span size='sm'>{t('invoices:InvoiceDetailHeader.action.refund_invoice', 'Issue refund')}</Span>
            </MenuItem>
          )}
          {invoiceActions.includes(InvoiceDetailHeaderMenuActions.Void) && (
            <MenuItem key={InvoiceDetailHeaderMenuActions.Void} onClick={onOpenChangeByMode(InvoiceDetailHeaderMenuActions.Void)}>
              <Span size='sm'>{t('invoices:InvoiceDetailHeader.action.mark_void', 'Mark as void')}</Span>
            </MenuItem>
          )}
          {invoiceActions.includes(InvoiceDetailHeaderMenuActions.Writeoff) && (
            <MenuItem key={InvoiceDetailHeaderMenuActions.Writeoff} onClick={onOpenChangeByMode(InvoiceDetailHeaderMenuActions.Writeoff)}>
              <Span size='sm'>{t('invoices:InvoiceDetailHeader.action.write_off', 'Write off')}</Span>
            </MenuItem>
          )}
          {invoiceActions.includes(InvoiceDetailHeaderMenuActions.Reset) && (
            <MenuItem key={InvoiceDetailHeaderMenuActions.Reset} onClick={onOpenChangeByMode(InvoiceDetailHeaderMenuActions.Reset)}>
              <Span size='sm'>{t('invoices:InvoiceDetailHeader.action.reset_to_saved', 'Reset to saved')}</Span>
            </MenuItem>
          )}
          {invoiceActions.includes(InvoiceDetailHeaderMenuActions.DownloadPdf) && (
            ...
            >
              <Span size='sm'>{t('invoices:InvoiceDetailHeader.action.download_pdf', 'Download PDF')}</Span>
            </MenuItem>
          )}
        </MenuList>
      </DropdownMenu>
