...
import { getInvoiceTermsFromDates } from '@features/invoices/InvoiceTermsComboBox/InvoiceTermsComboBox'
...
export const validateInvoiceForm = ({ value: invoice }: { value: InvoiceForm }, t: TFunction) => {
  ...
  const errors = []
  if (customer === null) {
    errors.push({ customer: t('invoices:InvoiceForm.formUtils.validation.customer_required', 'Customer is a required field.') })
  }

  if (!invoiceNumber.trim()) {
    errors.push({ invoiceNumber: t('invoices:InvoiceForm.formUtils.validation.invoice_number_required', 'Invoice number is a required field.') })
  }

  if (sentAt === null) {
    errors.push({ sentAt: t('invoices:InvoiceForm.formUtils.validation.invoice_date_required', 'Invoice date is a required field.') })
  }

  if (dueAt === null) {
    errors.push({ dueAt: t('invoices:InvoiceForm.formUtils.validation.due_date_required', 'Due date is a required field.') })
  }

  if (sentAt !== null && dueAt !== null && toCalendarDate(dueAt).compare(toCalendarDate(sentAt)) < 0) {
    errors.push({ dueAt: t('invoices:InvoiceForm.formUtils.validation.due_date_must', 'Due date must be after invoice date.') })
  }
  ...
  if (nonEmptyLineItems.length === 0) {
    errors.push({ lineItems: t('invoices:InvoiceForm.formUtils.validation.invoice_requires_empty', 'Invoice requires at least one non-empty line item.') })
  }

  nonEmptyLineItems.some((item) => {
    if (item.description.trim() === '') {
      errors.push({
        lineItems: t('invoices:InvoiceForm.formUtils.validation.invoice_incomplete_required', 'Invoice has incomplete line items. Please include required field: Description.'),
      })
      return true
    }
  })
  ...
  if (BD.isNegative(fromNonRecursiveBigDecimal(grandTotal))) {
    errors.push({ lineItems: t('invoices:InvoiceForm.formUtils.label.invoice_negative_total', 'Invoice has a negative total.') })
  }
