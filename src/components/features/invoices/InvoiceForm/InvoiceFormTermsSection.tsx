...
import { getDurationInDaysFromTerms, InvoiceTermsComboBox } from '@features/invoices/InvoiceTermsComboBox/InvoiceTermsComboBox'
    ...
    <HStack gap='xl' className='Layer__InvoiceForm__Terms'>
      <VStack gap='xs'>
              ...
              {customer => customer && (
                <HStack>
                  <Button variant='text' onPress={onClickEditCustomer}>
                    <Span size='sm'>{t('invoices:InvoiceForm.InvoiceFormTermsSection.action.edit_customer_details', 'Edit customer details')}</Span>
                  </Button>
                </HStack>
              )}
            </form.Subscribe>
        ...
        <form.AppField name='email'>
          {field => (
            <field.FormTextField label={t('common:label.email', 'Email')} inline className='Layer__InvoiceForm__Field__Email' isReadOnly />
          )}
        </form.AppField>
        <form.AppField name='address'>
          {field => (
            <field.FormTextAreaField label={t('invoices:InvoiceForm.InvoiceFormTermsSection.label.billing_address', 'Billing address')} inline className='Layer__InvoiceForm__Field__Address' isReadOnly />
          )}
        </form.AppField>
      </VStack>
      <VStack gap='xs'>
        <form.AppField name='invoiceNumber'>
          {field =>
            <field.FormTextField label={t('invoices:InvoiceForm.InvoiceFormTermsSection.label.invoice_number_label', 'Invoice number')} inline className='Layer__InvoiceForm__Field__InvoiceNo' isReadOnly={isReadOnly} />}
        </form.AppField>
        <form.Field
          name='terms'
          listeners={{
        ...
        >
          {field => <field.FormDatePickerField label={t('invoices:InvoiceForm.InvoiceFormTermsSection.label.invoice_date', 'Invoice date')} inline className='Layer__InvoiceForm__Field__SentAt' isReadOnly={isReadOnly} />}
        </form.AppField>
        <form.AppField
          name='dueAt'
          listeners={{
        ...
        >
          {field => (
            <field.FormDatePickerField label={t('invoices:InvoiceForm.InvoiceFormTermsSection.label.due_date', 'Due date')} inline className='Layer__InvoiceForm__Field__DueAt' isReadOnly={isReadOnly} />
          )}
        </form.AppField>
      </VStack>
    </HStack>
