...
import { negateNonRecursiveBigDecimal, type NonRecursiveBigDecimal } from '@schemas/common/nonRecursiveBigDecimal'
    ...
    <VStack className='Layer__InvoiceForm__Metadata' pbs='md'>
      <HStack justify='space-between' gap='xl'>
        ...
        <VStack className='Layer__InvoiceForm__TotalFields' fluid>
          <InvoiceFormTotalRow label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.subtotal', 'Subtotal')} value={subtotal} />
          <InvoiceFormTotalRow label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.discount', 'Discount')} value={negateNonRecursiveBigDecimal(additionalDiscount)}>
            <form.AppField name='discountRate'>
              {field => <field.FormNonRecursiveBigDecimalField label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.discount', 'Discount')} showLabel={false} mode='percent' isReadOnly={isReadOnly} />}
            </form.AppField>
          </InvoiceFormTotalRow>
          <InvoiceFormTotalRow label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.taxable_subtotal', 'Taxable subtotal')} value={taxableSubtotal} />
          <InvoiceFormTotalRow label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.tax_rate', 'Tax rate')} value={taxes}>
            <form.AppField name='taxRate'>
              {field => <field.FormNonRecursiveBigDecimalField label={t('invoices:InvoiceForm.InvoiceFormMetadataSection.label.tax_rate_title_case', 'Tax Rate')} showLabel={false} mode='percent' isReadOnly={isReadOnly} />}
            </form.AppField>
          </InvoiceFormTotalRow>
