...
} from '@features/invoices/InvoicePaymentForm/formUtils'
  ...
  const getErrorText = useCallback((reason: InvoicePaymentInvalidReason): string => {
    switch (reason) {
      case InvoicePaymentInvalidReason.AmountMustBePositive:
        return t('invoices:InvoicePaymentForm.validation.payment_amount_must', 'Payment amount must be greater than zero.')
      case InvoicePaymentInvalidReason.AmountExceedsOutstandingBalance:
        return t('invoices:InvoicePaymentForm.validation.payment_amount_max_outstanding', 'Payment amount cannot be greater than the outstanding invoice balance.')
      case InvoicePaymentInvalidReason.PaidAtRequired:
        return t('invoices:InvoicePaymentForm.validation.payment_date_required', 'Payment date is a required field.')
      case InvoicePaymentInvalidReason.PaidAtBeforeInvoiceDate:
        return t('invoices:InvoicePaymentForm.validation.payment_date_not_before_invoice', 'Payment date cannot be before the invoice date ({{invoiceDate}}).', {
      ...
      case InvoicePaymentInvalidReason.PaidAtInFuture:
        return t('invoices:InvoicePaymentForm.validation.payment_date_not_future', 'Payment date cannot be in the future.')
      case InvoicePaymentInvalidReason.MethodRequired:
        return t('invoices:InvoicePaymentForm.validation.payment_method_required', 'Payment method is a required field.')
      default:
        return ''
    }
