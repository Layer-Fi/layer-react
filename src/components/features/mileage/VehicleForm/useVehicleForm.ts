...
} from '@features/mileage/VehicleForm/formUtils'
  ...
  const getErrorText = useCallback((reason: VehicleFormInvalidReason): string => {
    switch (reason) {
      case VehicleFormInvalidReason.MakeAndModelRequired:
        return t('mileage:VehicleForm.useVehicleForm.validation.make_model_required', 'Make and model is a required field.')
      case VehicleFormInvalidReason.YearRequired:
        return t('mileage:VehicleForm.useVehicleForm.validation.year_required', 'Year is a required field.')
      case VehicleFormInvalidReason.YearRange:
        return t('mileage:VehicleForm.useVehicleForm.validation.year_range', 'Year must be between 1900 and {{maxYear}}.', {
