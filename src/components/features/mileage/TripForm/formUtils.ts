...
import { type TripForm, type TripPlace } from '@schemas/features/mileage/tripForm'
...
export const validateTripForm = ({ trip }: { trip: TripForm }, t: TFunction) => {
  ...
  const fields = {
    tripDate: required(t('mileage:TripForm.formUtils.validation.trip_date_required', 'Trip date is a required field.'))(tripDate)
      ?? dateNotInFuture(t('mileage:TripForm.formUtils.validation.trip_date_not_future', 'Trip date cannot be in the future.'))(tripDate),
    distance: positiveAmount(t('mileage:TripForm.formUtils.validation.distance_greater_than_zero', 'Distance must be greater than 0 miles.'))(distance),
    purpose: required(t('mileage:TripForm.formUtils.validation.purpose_required', 'Purpose is a required field.'))(purpose),
  }
