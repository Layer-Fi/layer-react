...
const MileageDeductionChartTooltipContent = ({ active, payload }: MileageDeductionChartTooltipContentProps) => {
  ...
  return (
    <ChartTooltipContent>
      ...
      <ChartTooltipRow label={t('mileage:MileageDeductionChart.label.miles_colon', 'Miles:')} value={<Span size='sm' variant='white'>{formatNumber(data.miles)}</Span>} />
      <ChartTooltipRow label={t('mileage:MileageDeductionChart.label.deduction_colon', 'Deduction:')} value={<MoneySpan status='success' amount={data.deduction} size='sm' />} />
    </ChartTooltipContent>
  )
}
