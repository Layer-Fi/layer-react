...
import { VehicleArchiveConfirmationModal } from '@features/mileage/VehicleArchiveConfirmationModal/VehicleArchiveConfirmationModal'
import { VehicleDeleteConfirmationModal } from '@features/mileage/VehicleDeleteConfirmationModal/VehicleDeleteConfirmationModal'
import { VehicleReactivateConfirmationModal } from '@features/mileage/VehicleReactivateConfirmationModal/VehicleReactivateConfirmationModal'
              ...
              <HStack gap='xs' className='Layer__VehicleCard__Header'>
                ...
                {vehicle.isPrimary && (
                  <Badge size={BadgeSize.SMALL} variant={BadgeVariant.INFO}>
                    {t('common:label.primary', 'Primary')}
                  </Badge>
                )}
                {isArchived && (
                  <Badge size={BadgeSize.SMALL} variant={BadgeVariant.WARNING}>
                    {t('common:state.archived', 'Archived')}
                  </Badge>
                )}
              </HStack>
            ...
            {isArchived
              ? (
                <Button variant='ghost' onPress={() => setIsReactivateModalOpen(true)}>
                  <RotateCcw size={16} />
                  {t('common:action.reactivate_label', 'Reactivate')}
                </Button>
              )
              : vehicle.isEligibleForDeletion
                ...
                : (
                  <Button variant='ghost' onPress={() => setIsArchiveModalOpen(true)}>
                    <Archive size={16} />
                    {t('common:action.archive_label', 'Archive')}
                  </Button>
                )}
          </HStack>
        </VStack>
