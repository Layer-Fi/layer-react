...
export const validateTimeEntryForm = ({ entry }: { entry: TimeEntryForm }, t: TFunction) => {
  ...
  if (date === null) {
    errors.push({ date: t('timeTracking:TimeEntryForm.formUtils.validation.entry_date_required', 'Entry date is a required field.') })
  }

  if (date && date.compare(today(getLocalTimeZone())) > 0) {
    errors.push({ date: t('timeTracking:TimeEntryForm.formUtils.validation.entry_date_not_future', 'Entry date cannot be in the future.') })
  }

  if (!(durationMinutes > 0) || !Number.isInteger(durationMinutes)) {
    errors.push({ durationMinutes: t('timeTracking:TimeEntryForm.formUtils.validation.duration_whole_minutes', 'Duration must be a whole number of minutes greater than zero.') })
  }

  if (!serviceId) {
    errors.push({ serviceId: t('timeTracking:TimeEntryForm.formUtils.validation.service_required', 'Service is a required field.') })
  }
