...
export const BankTransactionsHeaderMenu = ({ actions, isDisabled, isListView = false }: BankTransactionsHeaderMenuProps) => {
  ...
  const menuItems = useMemo<DropdownMenuItem[]>(() => {
    const items: DropdownMenuItem[] = [{
      ...
      label: t('bankTransactions:BankTransactionsHeader.action.download_transactions', 'Download transactions'),
    }]
    ...
    if (actions.includes(BankTransactionsHeaderMenuActions.BankTransactionsUploadWizard)) {
      items.push({
        ...
        label: t('bankTransactions:BankTransactionsHeader.action.upload_transactions_manually', 'Upload transactions manually'),
      })
    }
    ...
    if (actions.includes(BankTransactionsHeaderMenuActions.ManageCategorizationRules)) {
      items.push({
        ...
        label: t('bankTransactions:BankTransactionsHeader.action.manage_categorization_rules', 'Manage categorization rules'),
      })
    }
  ...
  return (
    <>
      <DataTableHeaderMenu
        ariaLabel={t('bankTransactions:BankTransactionsHeader.label.additional_bank_transaction_actions', 'Additional bank transactions actions')}
        items={menuItems}
