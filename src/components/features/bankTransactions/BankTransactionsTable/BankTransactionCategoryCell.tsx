...
export const BankTransactionCategoryCell = ({
  ...
  const submitButton = (
    <BankTransactionsSubmitButton
      ...
      isActive={isOpen}
      isError={isError}
      errorMessage={t('bankTransactions:BankTransactionsTable.error.approval_failed_check_connection', 'Approval failed. Check connection and retry in a few seconds.')}
    >
      {isError
  ...
  const expandButton = (
    <Button
      ...
      aria-label={t('bankTransactions:BankTransactionsTable.action.toggle_details', 'Toggle details')}
    >
      <Chevron open={isOpen} />
    </Button>
  )
