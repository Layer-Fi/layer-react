...
const GROUP_LABEL_I18N: Record<BankTransactionCategoryComboBoxGroup, { i18nKey: string, defaultValue: string }> = {
  [BankTransactionCategoryComboBoxGroup.TRANSFER]: translationKey('bankTransactions:BankTransactionCategoryComboBox.label.transfer_uppercase', 'TRANSFER'),
  [BankTransactionCategoryComboBoxGroup.MATCH]: translationKey('bankTransactions:BankTransactionCategoryComboBox.label.match_uppercase', 'MATCH'),
  [BankTransactionCategoryComboBoxGroup.SUGGESTIONS]: translationKey('bankTransactions:BankTransactionCategoryComboBox.label.suggestions_uppercase', 'SUGGESTIONS'),
  [BankTransactionCategoryComboBoxGroup.ALL_CATEGORIES]: translationKey('bankTransactions:BankTransactionCategoryComboBox.label.all_categories_uppercase', 'ALL CATEGORIES'),
}
...
export const getSuggestedCategoriesGroup = (bankTransaction: BankTransaction, t: TFunction) => {
  if (isLoadingSuggestions(bankTransaction)) {
    return {
      ...
      options: [new PlaceholderAsOption({ label: t('bankTransactions:BankTransactionCategoryComboBox.label.generating_suggestions_transaction', 'Generating suggestions for transaction...'), value: 'LOADING_SUGGESTIONS' })],
    }
  }
