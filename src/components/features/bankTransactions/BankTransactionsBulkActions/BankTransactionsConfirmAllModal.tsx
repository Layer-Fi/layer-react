...
export const BankTransactionsConfirmAllModal = ({ isOpen, onOpenChange, isMobileView = false }: BankTransactionsConfirmAllModalProps) => {
  ...
  return (
    <BaseConfirmationModal
      ...
      title={t('bankTransactions:BankTransactionsBulkActions.prompt.confirm_all_suggestions', 'Confirm all suggestions?')}
      content={(
        skippedCount === 0
          ? (
            <Span>
              {tPlural(t, 'bankTransactions:BankTransactionsBulkActions.label.this_will_confirm_count_transactions', {
                count,
          ...
          : (
            <VStack gap='xs'>
              <Span>
                {tPlural(t, 'bankTransactions:BankTransactionsBulkActions.label.actionable_count_transactions_will_be_confirmed', {
                  count,
                  displayCount: formatNumber(count),
              ...
              <Span>
                {tPlural(t, 'bankTransactions:BankTransactionsBulkActions.label.count_transactions_will_be_skipped', {
                  count: skippedCount,
      ...
      confirmLabel={t('bankTransactions:BankTransactionsBulkActions.action.confirm_all', 'Confirm all')}
      errorText={t('bankTransactions:BankTransactionsBulkActions.error.confirm_transactions', 'Failed to confirm transactions')}
      closeOnConfirm
