...
export const DeductionsSection = ({ form, isReadOnly, isDesktop }: TaxProfileFormSectionProps) => {
  ...
  return (
    <VStack className='Layer__TaxProfileForm__Section' gap='md'>
      <Heading level={3}>{t('taxEstimates:TaxProfileForm.label.deductions', 'Deductions')}</Heading>

      <form.AppField name='usConfiguration.deductions.homeOffice.useHomeOfficeDeduction'>
        {field => (
          <field.FormRadioGroupYesNoField
            label={t('taxEstimates:TaxProfileForm.label.use_simplified_home_office', 'Use simplified home office deduction?')}
          ...
          <form.AppField name='usConfiguration.deductions.homeOffice.homeOfficeArea'>
            {field => (
              <field.FormNonRecursiveBigDecimalField
                label={t('taxEstimates:TaxProfileForm.label.home_office_area_sq', 'Home office area (sq ft)')}
                isReadOnly={isReadOnly}
                placeholder={t('taxEstimates:TaxProfileForm.label.enter_area', 'Enter area')}
                mode='decimal'
                maxDecimalPlaces={3}
      ...
      <form.AppField name='usConfiguration.deductions.vehicle.useMileageDeduction'>
        {field => (
          <field.FormRadioGroupYesNoField
            label={t('taxEstimates:TaxProfileForm.label.use_standard_mileage_deduction', 'Use standard mileage deduction?')}
            isReadOnly={isReadOnly}
            {...desktopFieldProps}
          />
        )}
