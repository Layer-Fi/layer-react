...
import { LinkedAccountToConfirm } from '@features/linkedAccounts/LinkedAccountToConfirm/LinkedAccountToConfirm'
  ...
  return (
    <div className={CLASS_NAME}>
      <VStack slot='center' gap='xs'>
        <Heading size='sm' align='center'>{t('linkedAccounts:LinkAccounts.label.accounts_successfully_linked', 'Accounts Successfully Linked')}</Heading>
        <P variant='subtle' align='center'>{t('linkedAccounts:LinkAccounts.label.link_more_accounts_any_time', 'You can link more accounts at any time from the Bank Transactions section')}</P>
      </VStack>
    </div>
  )
}
...
export function LinkAccountsConfirmationStep() {
  ...
  const getSubmitButtonText = useCallback((
    ...
    if (confirmedCount === totalCount) {
      return tPlural(t, 'linkedAccounts:LinkAccounts.action.confirm_accounts', {
        count: totalCount,
        one: 'Confirm Account',
    ...
    if (confirmedCount === 0) {
      return tPlural(t, 'linkedAccounts:LinkAccounts.action.exclude_all_accounts', {
        count: totalCount,
        one: 'Exclude Account',
    ...
    return tPlural(t, 'linkedAccounts:LinkAccounts.action.confirm_accounts_selected', {
    ...
    <VStack gap='lg'>
      <VStack gap='2xs'>
        <Heading level={3} size='sm'>
          {t('linkedAccounts:LinkAccounts.prompt.which_accounts_for_business', 'Which accounts do you use for businesses?')}
        </Heading>
        <P variant='subtle'>
          {t('linkedAccounts:LinkAccounts.label.deselect_unused_accounts', 'Please deselect any accounts you don’t use for your business.')}
        </P>
      </VStack>
