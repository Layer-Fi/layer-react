    ...
    if (!isValidReceiptFile(file)) {
      ...
      setReceiptUrls(prev => [
        ...prev,
        {
          ...
          error: t('bankTransactions.invalidFileTypePleaseUploadAnImageOrPdf', 'Invalid file type. Please upload an image or PDF.'),
        },
      ])
      return
    }
      ...
      setReceiptUrls(prev =>
        prev.map((url) => {
          if (url.id === id) {
            return {
              ...url,
              error: t('bankTransactions.failedToUpload', 'Failed to upload'),
              status: 'failed' as const,
            }
          }
        ...
        prev.map((url) => {
          if (url.id === document.id) {
            return {
              ...url,
              status: 'failed',
              error: t('bankTransactions.failedToDelete', 'Failed to delete'),
            }
          }
