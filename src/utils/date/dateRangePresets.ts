...
import { clampToAfterActivationDate, clampToPresentOrPast, type DateRange, isSameDateRangeDayGranularity } from '@utils/date/dateRange'
...
const clampToValidRange = (range: DateRange, { now, activationDate }: DatePresetContext): DateRange => {
  ...
  return {
    startDate: clampToAfterActivationDate(rawStartDate, startOfDay(activationDate)),
    endDate: clampToPresentOrPast(rawEndDate, endOfDay(now)),
  }
}
