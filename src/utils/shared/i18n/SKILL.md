...
Locale → currency mapping lives in `@utils/shared/i18n/number/currency` (`getCurrencyForLocale`):
`en-US` → USD, `fr-CA` → CAD. That module also has `getLocaleCurrencySymbol` and
`transformCurrencyValue` for currency *input* fields, and `@utils/shared/i18n/number/input` has
...
Use a `DateFormat` member from `@utils/shared/i18n/date/patterns` — **never a custom format string**.
