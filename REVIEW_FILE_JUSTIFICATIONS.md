...
- **Default behavior:** Global `dateMode` initializes to `'month'`, so the Monthly experience is the default landing mode on Overview.
...
- **What Changed:** Implemented stacked date controls, updated wrapper classes, and added conditional rendering to toggle between the original P&L Chart ('month' mode) and the new `PnLHorizontalBarChart` ('custom' mode). In custom mode, the Revenue/Expenses/Net Profit/Transactions-to-Review summary banners are now hidden entirely. The `onTransactionsToReviewClick` callback is forwarded to the `PnLHorizontalBarChart` for its CTA. The `showPercentageChanges` prop was removed. Added a mode-scoped view class (`Layer__AccountingOverviewView--range`) so responsive header behavior can differ between Monthly (default) and Range modes.
- **Why We Did It:** Brings the Accounting Overview into alignment with the new global capabilities. Hiding the top-level summaries in custom mode avoids redundancy since the horizontal bar chart now displays its own Net Profit header and contextual transaction CTA. Conditionally rendering the charts maintains the original behavior for standard month views while introducing the detailed bookkeeping breakdown for custom date ranges. The mode-scoped class prevents premature header wrapping in default Monthly mode while still allowing earlier controlled reflow for Range controls.

### `src/styles/accounting_overview.scss`
- **What Changed:** Added specific layout configurations for the Overview header, including vertical padding, structural normalization classes, and container query collapse breakpoints. Refined responsiveness to use staged, mode-aware breakpoints: Range headers collapse earlier (`max-width: 1040px`), Range controls move to a two-row tablet layout with a full-width 3-column date row (`max-width: 900px`), and only collapse to single-column controls on small mobile (`max-width: 640px`). Also removed horizontal-scroll fallback from the date selection wrapper to avoid squeeze/scroll transitions.
- **Why We Did It:** Layout isolation. The Overview header operates spatially differently than Report headers. These rules ensure the stacked date controls breathe appropriately and stack cleanly without requiring messy inline React styles. Mode-aware breakpoints preserve the default Monthly one-row desktop-to-tablet experience (avoiding premature drop-down to a second row) while still keeping the wider Range control set stable and readable across viewport transitions.

### `src/components/PnLHorizontalBarChart/PnLHorizontalBarChart.tsx` (New)
- **What Changed:** Built a net-new categorized vs uncategorized horizontal stacked bar chart. Exports a detached legend. Displays a Net Profit amount with label under the header date range. Revenue and Expenses bar rows are rendered inside a single unified container showing only the label, dollar amount, and bar — the per-row `% categorized` text and uncategorized transaction count Badge have been removed. Instead, a `TransactionsToReview` banner (showing total pending count) is rendered inline on the same row as Net Profit via the existing `TransactionsToReview` component. The previous bottom-right "Review transactions" CTA has been removed in favor of this consolidated placement.
- **Why We Did It:** Product feedback indicated the per-bar categorization stats were noisy and redundant. Consolidating transaction review status into a single `TransactionsToReview` banner next to Net Profit gives a cleaner layout with a single, prominent call-to-action. Reusing the existing `TransactionsToReview` component avoids duplication and keeps the transaction count data sourced from `ProfitAndLossContext` in one place.

### `src/components/PnLHorizontalBarChart/pnLHorizontalBarChart.scss` (New)
- **What Changed:** Added styling for the new horizontal chart: unified bars container with single border/gradient background, separator between Revenue and Expenses rows, Net Profit label styling, net-profit-row layout for inline TransactionsToReview placement, legend text, and container queries for mobile stacking. Removed styles for the deleted status-group, completion text, and CTA button.
